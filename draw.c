...
void add_sphere( struct matrix * edges,
                 ...
                 double r, int step ) {
    struct matrix *surfacepoints = generate_sphere(cx,cy,cz,r,step);
    int i;
    for (i=0;i<surfacepoints->cols;i++) {
      double x = surfacepoints->m[0][i];
      double y = surfacepoints->m[1][i];
      double z = surfacepoints->m[2][i];
...
/*======== void generate_sphere() ==========
 ...
 struct matrix * generate_sphere(double cx, double cy, double cz, double r, int step ) {
  ...
  struct matrix *surfacepoints = new_matrix(4, 4);

  double x1, y1, z1, t, t2;
  int i,j;
  ...
  for (j=1; j<=step; j++) {
    ...
    for (i=1; i<=step/2; i++) {
      ...
      y1 = r * sin(2 * M_PI * t) * cos(2*M_PI*t2) + cy;
      z1 = r * sin(2 * M_PI * t) * sin(2*M_PI*t2) + cz;

      add_point(surfacepoints, x1, y1, z1);
    }
...
void add_torus( struct matrix * edges, double cx, double cy, double cz, double r1, double r2, int step ) {
  struct matrix *surfacepoints = generate_torus(cx,cy,cz,r1,r2,step);
  int i;
  for (i=0;i<surfacepoints->cols;i++) {
    double x = surfacepoints->m[0][i];
    double y = surfacepoints->m[1][i];
...
struct matrix * generate_torus( double cx, double cy, double cz, double r1, double r2, int step ) {
    ...
    for (j=1; j<=step; j++) {
      ...
      for (i=1; i<=step; i++) {
        t = (double)i/step;
        x1 = cos(2*M_PI*t2) * (r1*cos(2*M_PI*t)+r2) + cx;
        y1 = r1 * sin(2*M_PI*t) + cy;
        z1 = -sin(2*M_PI*t2) * (r1*cos(2*M_PI*t)+r2) + cz;
